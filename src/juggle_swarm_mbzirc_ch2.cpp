...
{ 
  ...
  // gripper_on.call(srv);
    ...
    vector < vector <float> > pick_x(3);
    vector < vector <float> > pick_y(3);
    vector < vector <float> > pick_z(3);

    vector < vector <float> > drop_x(3);
    vector < vector <float> > drop_y(3);
    vector < vector <float> > drop_z(3);

    float Pick_X = 0.0, Pick_Y = 0.0;
    float Drop_X = 0.0, Drop_Y = 0.0;
  ...
  cout << "iz" << endl;
    ...
    geometry_msgs::PoseStamped air_pose;
    ...
    int ready_to_pick = -1;
    int ready_to_drop = -1;
    int brick_pick_drone = -1;
    int brick_drop_drone = -1;
    int air_space_drone = 0;

    while (ros::ok())
    {
        if (brick_pick_drone != -1) {
            if (sequence[pickup_no] == 'R') {
                Pick_X = 16;
                Pick_Y = -4;
            } else if (sequence[pickup_no] == 'G') {
                Pick_X = 16;
                Pick_Y = -16;
            } else if (sequence[pickup_no] == 'B') {
                Pick_X = 16;
                Pick_Y = -10;
            }
            pick_x[brick_pick_drone].push_back(Pick_X);
            pick_y[brick_pick_drone].push_back(Pick_Y);
        } else {
            brick_pick_drone = air_space_drone;
            air_space_drone = 1;
            ready_to_pick = brick_pick_drone;
        }

        if (brick_drop_drone != -1) {
            Drop_X = brick_initial_posx;
            Drop_Y = brick_initial_posy + drop_no * brick_length;
            if (Drop_Y >= 13) {
                Drop_Y = 13;
                Drop_X = brick_initial_posx + (drop_no - 5) * brick_width;
            }
            drop_x[brick_drop_drone].push_back(Drop_X);
            drop_y[brick_drop_drone].push_back(Drop_Y);
        } else {
            if (brick_picked_up(brick_pick_drone)) {
                ready_to_drop = brick_pick_drone;
                brick_pick_drone = air_space_drone;
            }
        }

        if (brick_picked(brick_pick_drone)) {
            ready_to_drop = brick_pick_drone;
            brick_pick_drone = air_space_drone;
            picking_no = (picking_no + 1) % 7;
            air_space_drone = -1;
        } else {
