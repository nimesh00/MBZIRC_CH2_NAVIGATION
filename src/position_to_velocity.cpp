...
// float sigmoid_modified(float x, float softening_factor=1) {
//     return (1.0 / (softening_factor + exp(-x))) - (1.0 / (1 + softening_factor));
// }
...
{
    ros::init(argc, argv, "velocity_control_px4_tanh");
    ros::NodeHandle nh;

    int no_of_UAV = 3;

    array<ros::Subscriber, no_of_UAV> coords_sub;
    ...
    while(ros::ok()){
        ...
        pose1.velocity.x = velocity_factor * tanhl(unit_vec_x / 5);
        pose1.velocity.y = velocity_factor * tanhl(unit_vec_y / 5);
        pose1.velocity.z = velocity_factor * tanhl(unit_vec_z / 5);
