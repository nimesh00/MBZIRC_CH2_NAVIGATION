...
{ 
    ...
    for (int i = 0; i < no_of_uav; i++) {
        for (r = 2; r < 50; r += FOV) {
            if (r % 4 != 0) {
                for (theta = i * angle; theta <= angle * (i + 1); theta += 30) {
                    float theta_degrees = theta * (M_PI / 180);
                    array<float, 2> coordinates;
                    coordinates[0] = r * cos(theta_degrees);
                    coordinates[1] = r * sin(theta_degrees);
                ...
                for (theta = angle + i * angle; theta >= i * angle; theta -= 30) {
                    float theta_degrees = theta * (M_PI / 180);
                    array<float, 2> coordinates;
                    coordinates[0] = r * cos(theta_degrees);
                    coordinates[1] = r * sin(theta_degrees);
    ...
    cout << "Detecting resource zones!!" << endl;
    ...
    while(ros::ok()) {
        ...
        for (int i = 0; i < no_of_uav; i++) {
            ...
            /* Hard coded block. Remove it when using callback function for detection flags. */
            if (current_target[i].pose.position.x <= -15 && current_target[i].pose.position.y >= 9) {
                cout << "Drop Zone Detected!!" << endl;
                detected_drop_zone = 1;
            }

            if (current_target[i].pose.position.x >= 16 && current_target[i].pose.position.y <= -4) {
                cout << "Pickup Zone Detected!!" << endl;
                detected_pickup_zone = 1;
            }
            /* ***************************************************************************** */
    ...
    cout << "Detected Required Zones!! Starting Construction!!" << endl;

    float brick_length=1,brick_width=1;
    // this comes from the vision callback
